...
위 코드는 특정 메모리의 이름을 ``a``로 정한 것이며, 선언한 메모리에 어떤 값도 지정하지 않았기 때문에 아직 아무런 값을 가지지 않는다.  
...
>> 출력값 : 고양이는 귀엽다.
```
위 코드는 ``고양이는 귀엽다``라는 값을 입력하면 이를 ``num``값에 저장하고 ``출력값 : ``에 더하여 출력한다.  
이같이 입력한 값을 저장하는 위치를 지정함으로써 이를 사용할 수 있게 된다.
>``input(n)``은 괄호 안에 값을 입력할 수 있는데 이는 마찬가지로 값을 입력받으나 추가적으로 ``print(n)``의 기능도 수행한다.
...
>> 참
```
이런 식으로 비교 연산자를 사용하여 참과 거짓을 판단하는 것이며, 다양한 비교 연산자를 사용할 수 있다.
## 3.1.2. else
